...
//        }
    ...
    public KeyStore loadTokenKeyStore(String ksPass, UserAgent userAgent, String activeProvider) throws Exception {
        ...
        //Gemalto
        GemaltoAutoConfigFile fepass4 = new GemaltoAutoConfigFile(cfgUtil);
        configFilePath = tempDir + "gemalto.cfg";
        System.out.println("configFilePath:" + configFilePath);
        fepass4.createConfigFile(configFilePath, userAgent);
        File gemaltoConf = new File(configFilePath);
        ...
        //gemalto
        SunPKCS11 sunPKCS11gemalto = null;
        if (gemaltoConf.length() != 0L) {

            if (Security.getProvider("SunPKCS11-gemalto") == null) {
                //BUG win XP se usa la canonica
                String path = null;
                try {
                    path = gemaltoConf.getCanonicalPath();
                } catch (Exception w) {
                    path = gemaltoConf.getPath();
                }
                try {
                    sunPKCS11gemalto = new sun.security.pkcs11.SunPKCS11(path);

                    Security.addProvider(sunPKCS11gemalto);
                } catch (ProviderException pe) {
                    sunPKCS11gemalto = null;
                    pe.printStackTrace();
                }

            } else {
                sunPKCS11gemalto = (SunPKCS11) Security.getProvider("SunPKCS11-gemalto");
            }
        }

        if ((!hayMod) && (sunPKCS11safenet == null) && (sunPKCS11 == null) && (sunPKCS11epass2003 == null) && (sunPKCS11epass3003auto == null) && (sunPKCS11gemalto == null)) {
    ...
    public static void main(String[] args) {
        try {
            ...
            certs = storeMg.getSignCertificateFromPKCS11("1234", "", PKCS11CertStoreImpl.PKCS11CertStoreType.ETOKEN, "");
            /*
