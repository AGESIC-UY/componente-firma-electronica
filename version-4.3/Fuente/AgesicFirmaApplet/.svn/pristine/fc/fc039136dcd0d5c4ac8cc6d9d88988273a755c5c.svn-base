 ...
 */
package uy.gub.agesic.firma.cliente.store.pkcs11.acscrypto;
 ...
 */
public class ACSCryptoMateConfigFile {

    private ConfigurationUtil cfgUtil;
    final static String PATHS_SEPARATOR = ";";

    public ACSCryptoMateConfigFile(ConfigurationUtil cfgUtil) {
    ...
    public void createConfigFile(String path, UserAgent userAgent) throws IOException {
        ...
        if (!f.exists()) {
            System.out.println("CREANDO ACSCryptoMateConfigFile CONFIG FILE EN " + path + "-" + userAgent.getOperatingSystem().getGroup() + "-" + isWindows);
            f.createNewFile();
        } else {
        ...
        File flib;
        String name = "aCSCryptoMate";



        if (isWindows || userAgent.getOperatingSystem().getGroup().equals(OperatingSystem.WINDOWS)) {
            String paths = cfgUtil.getValue(AppletParams.PARAM_ACSCryptoMate_WIN);
            ...
            if (userAgent.getOperatingSystem().getGroup().equals(OperatingSystem.LINUX)) {
                ...
                paths = cfgUtil.getValue(AppletParams.PARAM_ACSCryptoMate_LIN);
            } else {
                //MAC
                paths = cfgUtil.getValue(AppletParams.PARAM_ACSCryptoMate_MAC);
            }
